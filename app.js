const API_BASE = "/api";
const STATUSES = ["Offen", "Kontaktiert", "Im Gespräch", "Abgeschlossen", "Abgelehnt"];
...
function statusClass(status) {
  return "status-" + status.replace(/\s+/g, "-").replace("Gespräch", "Gespraech");
}
...
async function addSampleLead() {
  ...
  await api("/leads", {
    ...
    body: JSON.stringify({
      ...
      prio: "HOCH",
      status: "Offen",
      notiz: "Demo-Lead für die CRM-Ansicht",
      source: "Research",
      createdAt: now,
      updatedAt: now
    })
...
async function quickAdvanceLead(id) {
  ...
  if (!lead) return;
  const order = ["Offen", "Kontaktiert", "Im Gespräch", "Abgeschlossen"];
  ...
  await api(`/leads/${encodeURIComponent(id)}`, {
    ...
    body: JSON.stringify({
      ...lead,
      status: next,
      history: [{ date: new Date().toISOString(), text: "Status geändert zu " + next }, ...(lead.history || [])]
    })
  });
  await load();
